---
name: product-rnd-report-gen
description: Generate professional Product Innovation Research & Development reports as HTML. Conducts research, creates HTML reports, and uploads for sharing.
...
- Use uploaded image URLs (not local paths) - images should be uploaded first, then URLs embedded in HTML
...
5. **Upload Images & Update HTML**
   - Upload all images: `{baseDir}/scripts/upload.sh --quiet product-image-*.png`
   - Replace image placeholders in HTML with uploaded URLs
   - Save updated HTML: `{base_dir}/{Project-Name}/report.html`

6. **Upload Final Report**
   - Upload HTML report: `{baseDir}/scripts/upload.sh --quiet {base_dir}/{Project-Name}/report.html`
...
- After writing each section, verify the file was saved correctly before proceeding to the next section